...
data "oci_core_ipsec_connection_tunnels" "ipsec_ix2105_tunnels" {
  ipsec_id = oci_core_ipsec.ipsec_ix2105.id
}
...
resource "oci_core_ipsec_connection_tunnel_management" "ipsec_ix2105_config" {
  for_each = { 0 = "Alpha", 1 = "Beta" }
  ipsec_id  = oci_core_ipsec.ipsec_ix2105.id
  tunnel_id = data.oci_core_ipsec_connection_tunnels.ipsec_ix2105_tunnels.ip_sec_connection_tunnels[each.key].id
  display_name = "NEC IX 2105-1 IPSec Tunnel - ${each.value}"
  routing = "BGP"
  ike_version = "V2"
  bgp_session_info {
    customer_bgp_asn = "65000"
    customer_interface_ip = "10.9.16.${each.key * 2 + 4}/31"
    oracle_interface_ip = "10.9.16.${each.key * 2 + 1 + 4}/31"
  }
  oracle_can_initiate = "RESPONDER_ONLY"
  phase_one_details { lifetime = 28800 }
  phase_two_details { lifetime = 7200 }
